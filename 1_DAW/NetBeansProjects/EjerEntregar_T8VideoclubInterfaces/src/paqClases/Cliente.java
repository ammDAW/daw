...
//OTROS METODOS    
    public void alquilarProducto(Producto pr)throws NoMas5ProductosAlquiladosPorClientesException{ //añade objetos productos al ArrayList
        if(this.listaProductos.size()==5)
            throw new NoMas5ProductosAlquiladosPorClientesException();
        pr.alquilar();//ejercicio interfaces
        this.listaProductos.add(pr);
    }
    
    public void devolverProducto(Producto pr){
        if (this.listaProductos.contains(pr)){
            this.listaProductos.remove(pr);
            pr.devolver();//ejercicio interfaces
        }
        else{System.out.println(pr.nombre+" NO se puede devolver debido a que "+this.nombre+" no lo tiene alquilado");}
    }
